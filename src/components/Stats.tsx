...
function StatCard({ number, label, suffix = '', isArabic = false }: StatProps & { isArabic?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Accent Bar on Hover */}
      <div className="absolute left-0 top-0 h-full w-1 bg-gradient-to-b from-[#DE3B34] to-[#F0716B] opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-l-xl" />
      {/* Content */}
      <div className="relative text-center py-14 px-6 flex flex-col items-center justify-center h-full">
        <div
          ...
          style={{ fontFamily: 'Georgia, serif', color: '#DE3B34', textShadow: '0 2px 8px rgba(222,59,52,0.15)' }}
        >
          {displayValue()}
        </div>
        <p className={`text-xs md:text-base font-semibold tracking-[0.12em] leading-relaxed max-w-[220px] mx-auto text-gray-800 ${isArabic ? 'normal-case' : 'uppercase'}`}>
          {label}
        </p>
      </div>
    </div>
      ...
      <div className="relative max-w-[1200px] mx-auto">
        ...
        <div className="text-center mb-20">
          <div className="inline-flex items-center gap-3 mb-8">
            <div className="h-[2px] w-12 bg-[#DE3B34]" />
            <span className="text-xs uppercase tracking-[0.25em] font-semibold text-[#c73731]">
              {pageContent.badge}
            </span>
            <div className="h-[2px] w-12 bg-[#DE3B34]" />
          </div>
          ...
          <div className="flex justify-center">
            <div className="h-[3px] w-24 rounded-full bg-gradient-to-r from-transparent via-[#DE3B34] to-transparent" />
          </div>
        </div>
